...
const Matrimony = (props) => {
    ...
    const [isMatrimonyShow,setIsMatrimonyShow]=useState(true);
    const loadMoreData = () => {
        ...
        var reqData = { //if array of queries pass [] outside
            query_type: 'query',
            query: "select *,ROUND(DATE_FORMAT(FROM_DAYS(DATEDIFF(now(),dob)), '%Y')) AS age from members where is_matrimony_member='1' and status='1' and gender<>'" + loggedUserData.gender + "'" + ageWhereClause.current + context.psGlobal.getWhereClause(filterColumns.current, false) + "  order by created_date desc LIMIT " + currentFetchedRecords.current + "," + recordsPerRequest,
            // table:'test',
            // where:{id:'1'},
    ...
    useEffect(() => {
        ...
        loadCurrentUser();
        resetResult();
        //if(parseInt(context.customerUser.is_matrimony_member)!==1)
           // setIsMatrimonyShow(false);
    ...
    const resetResult = () => {
        ...
        var reqData = { //if array of queries pass [] outside
            query_type: 'query',
            query: "select count(*) as count from members where  is_matrimony_member='1' and status='1'  and gender<>'" + loggedUserData.gender + "'" + ageWhereClause.current + context.psGlobal.getWhereClause(filterColumns.current, false),
            // table:'test',
            // where:{id:'1'},
            //values:{name:'senthil',age:'13'}
    ...
    return (
        <>
            <div class="main-content right-chat-active" >
                ...
                {!isViewProfile  && isMatrimonyShow && (<Spin spinning={initLoading} >
                        <Card
