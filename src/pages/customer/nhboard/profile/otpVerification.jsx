...
const OtpVerification = (props) => {
    ...
    const loadCustomerOTP = () => {
        var reqData = [{
            ...
            query: "select mobile_otp from hospital_members where id='" + context.customerUser.id + "' and status='1'"
        },
        {
            query_type: 'query', //query_type=insert | update | delete | query
    ...
    const onResendClick = () => {
        ...
        var smsData = {
            ...
            messages: [
                {
                    mobile_no: context.customerUser.mobile,
    ...
    const onOTPClose=()=>{
        if (window.confirm('Verify OTP,or Are you sure to logout?') == true) {
            setvisibleModal(false); context.customerLogout();
            ...
            >
                <Spin spinning={loading}>
                    ...
                    <Row gutter={16} style={{ marginTop: '10px' }}>
                        ...
                        <Col className='gutter-row' xs={24} xl={16} >: <span style={{ color: cyan[6], fontWeight: 'bold' }}>{dataItem.mobile}</span>
                        </Col>
                    </Row>
                    {
                        !showOTPInput && (<Row gutter={16} style={{ marginTop: '10px', fontSize: '15px', color: cyan[7] }}>
                        <Col xs={24} xl={24}>
                            <Space><MyButton type="primary" onClick={onResendClick}>Sent OTP</MyButton>
                            {/*<MyButton type="outlned" onClick={()=>setvisibleModal(false)}>Skip </MyButton>*/}</Space>
                        </Col>
                    </Row>)
                    }
