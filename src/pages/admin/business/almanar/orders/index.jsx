...
const ProductOrders = (props) => {
    ...
    const [heading] = useState('Online Order');
            ...
            render: (item) => <Space>
               ...
               {/*  <MyButton type="outlined" size="small" shape="circle"
                ...
                {context.isAdminResourcePermit(userId, 'products.delete-product') && (<DeleteButton type="outlined" size="small" shape="circle" color={red[7]} onFinish={() => { setCurAction("list"); setRefreshTable(prev => prev + 1); }}
                    ...
                    dataItem={{ id: item.id, first_name: item.first_name, order_id: item.order_id, mobile: item.mobile }}
    ...
    const addEditComponents = () => {
        if (curAction === "view")
            return <ViewJewelProduct formItemLayout={formItemLayout} viewIdOrObject={viewOrEditData.id} onListClick={onAddEditListClick} userId={userId} />
    ...
    return (
        <>
            ...
            >
                {
                    !context.isMobile && (<>
                        ...
                        <Card title="Orders" extra={context.isAdminResourcePermit(userId, 'products.add-product') ? <><Space>
                         {/*    <MyButton onClick={onAddClick} ><i className="fa-solid fa-plus pe-2" ></i>Add Product</MyButton> */}
                       ...
                       {/*  <MyButton onClick={onExcelClick} ><i className="fa-solid fa-file-excel pe-2" ></i>Excel</MyButton> */}
                            ...
                            <PaginatedTable
                                ...
                                countQuery="select count(*) as count from orders where status=1 "
                                listQuery="select p.*,@rownum:=@rownum+1 as row_num from orders p CROSS JOIN (SELECT @rownum:={rowNumberVar}) c where p.status=1 order by p.id desc"
                                itemsPerPage={20}
                            />
                    ...
                    context.isMobile && (<>
                        ...
                        <div style={{ display: (curAction === "list") ? 'block' : 'none' }}>
                            ...
                            <AvatarMobileInfiniteList
                                ...
                                refresh={refreshTable}
                                countQuery="select count(*) as count from orders where status=1 "
                                listQuery="select p.*  from orders p  where p.status=1  order by p.id desc"
            ...
            <Modal
               ...
               {/*  <MyButton type="outlined" size="small" onClick={onDescriptonUpdate} >Update </MyButton> */}
                
                 Descripiton : {viewOrEditData?.product_name}</>}
