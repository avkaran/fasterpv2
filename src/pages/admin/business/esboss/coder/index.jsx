...
const CodeGenerator = (props) => {
    ...
    const LoadTables = (project) => {
        ...
        axios.post(project.api_url, form).then(res => {
            if (parseInt(res['data'].status) === 1) {
                let tNames = [];
                let treeItems = [];
                res['data'].data[0].forEach((item) => {
                    let tname = item['TABLE_NAME'];
                    let tColumns = res['data'].data[1].filter((col) => col.TABLE_NAME === tname)
                    let table = {
                    ...
                    <Row gutter={16}>
                        ...
                        <Col className='gutter-row' xs={24} xl={6} >
                            <Select
                                ...
                                onChange={(value) => setSelProjectId(value)}
                            ...
                            >
                                {
                                    projects.map(item => {
                                        return <Select.Option value={item.id}>{item.project_name}</Select.Option>
                                    })
                                }
                        ...
                        <Col className='gutter-row' xs={24} xl={4} >
                            <MyButton type="primary" onClick={onLoadTablesClick}>Load Tables</MyButton>
                        </Col>
                    </Row>
                </Card>
