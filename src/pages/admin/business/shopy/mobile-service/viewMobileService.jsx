...
const ViewMobileService = (props) => {
    ...
    const loadViewData = (id) => {
        ...
        var reqData = {
            query_type: 'query',
            query: "select * from tour_packages where status=1 and id=" + id
        };
                    ...
                    >
                        <Row gutter={3}>
                            <Col className='gutter-row' xs={24} xl={formItemLayout==='one-column'?24:12}>
                                <FormViewItem label="Under Construction" colon={formItemLayout!=='two-column-wrap' || context.isMobile}>{"Under Construction"}</FormViewItem>
                            </Col>
                           {/*  <Col className='gutter-row' xs={24} xl={formItemLayout==='one-column'?24:12}>
                                <FormViewItem label="Categories" colon={formItemLayout!=='two-column-wrap' || context.isMobile}>{viewData.categories}</FormViewItem>
                            </Col>
                            <Col className='gutter-row' xs={24} xl={formItemLayout==='one-column'?24:12}>
                                <FormViewItem label="Package Image" colon={formItemLayout!=='two-column-wrap' || context.isMobile}>
                                {viewData.package_image && (<Image height={100} src={context.baseUrl  +'/cloud-file/'+ encodeURIComponent(encodeURIComponent(viewData.package_image))} ></Image>)}
                                </FormViewItem>
                            </Col>
                            <Col className='gutter-row' xs={24} xl={formItemLayout==='one-column'?24:12}>
