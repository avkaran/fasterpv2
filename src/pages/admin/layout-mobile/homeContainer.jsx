...
import { faBars, faBell, faChevronLeft, faClose } from '@fortawesome/free-solid-svg-icons';
import toast from 'react-hot-toast';
const HomeContainer = (props) => {
	const context = useContext(PsContext);
	const navigate = useNavigate();
	const { pathname } = useLocation();
	const { role, userId, customHeader, bottomMenues, showNav, ...other } = props;
	...
	useEffect(() => {
		if (showNav && showNav === 'open')
			setVisibleSideBar(true)
		if (showNav && showNav === 'close')
			setVisibleSideBar(false)
	}, [showNav]);
	const log_out = () => {
		toast((t) => (
