...
const BrokerDashboard = (props) => {
    ...
    const tableColumns = [
        ...
        },
        {
            ...
            //dataIndex: 'employee',
            key: 'employee',
            render: (item) => <MyButton type='primary' shape="round" color={green[5]} href={"#/" + userId + "/admin/members/logs-by-action/" + item.action + "/broker"}>{item.employee}</MyButton>,
        },
    ...
    const loadTableCountData = () => {
        var reqData = [
            {
                query_type: 'query',
                query: "SELECT log_name,logged_type,count(distinct ref_id) as count FROM logs where date(log_time)='" + dayjs().format("YYYY-MM-DD") + "' and logged_type='broker' and logged_by='" + context.adminUser(userId).id + "' GROUP by log_name"
            },
        ]
        context.psGlobal.apiRequest(reqData, context.adminUser(userId).mode).then((res) => {
            var tCountData = [
            ...
            tCountData.forEach((item, index) => {
                ...
                var fEmployee = res[0].find(obj => obj.log_name === item.action && obj.logged_type === "broker");
