...
import AddPackage from './addEditSms';
import AddEditSms from './addEditSms';
import ViewBusinessName from './viewSms';
...
const {userId}=useParams();
    ...
    const tableColumns = [
       ...
       
        {
            ...
            key: 'is_unicode',
            render: (item) => <Tag color={item.is_unicode === 'Active' ? 'green' : 'red'} style={{ fontWeight: 'bold' }}>{item.is_unicode}</Tag>,
        },
        {
            title: 'Actions',
            // dataIndex: 'actions',
            ...
            render: (item) => <Space>
                ...
                title={heading}
                    ...
                    dataItem={{ id: item.id,template_for:item.template_for, template_id: item.template_id, used_variables: item.used_variables }}
            ...
            >
                ...
                <Card title={heading+" List"} extra={<MyButton onClick={onAddClick} ><i className="fa-solid fa-plus pe-2" ></i>Add {heading}</MyButton>} style={{display:(curAction === "list" || isModal)?'block':'none'}}>
                        ...
                        <PaginatedTable
                         ...
                         countQuery="select count(*) as count from sms_templates where status=1 "
                         listQuery="select *,@rownum:=@rownum+1 as row_number from sms_templates CROSS JOIN (SELECT @rownum:={rowNumberVar}) c where status=1 "
                         itemsPerPage={20}
                        />
