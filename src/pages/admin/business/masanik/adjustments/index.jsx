...
const Adjustments = (props) => {
    ...
    const tableColumns = [
        ...
        },
        {
            ...
            render: (item) => <Space>
                ...
                {context.isAdminResourcePermit(userId, 'alldisputes.edit-dispute') && (<MyButton type="outlined" size="small" shape="circle" color={blue[7]}
                    onClick={() => onEditClick(item)}
                ><i class="fa-solid fa-pencil"></i></MyButton>)}
                {context.isAdminResourcePermit(userId, 'alldisputes.delete-dispute') && (<DeleteButton type="outlined" size="small" shape="circle" color={red[7]} onFinish={() => { setCurAction("list"); setRefreshTable(prev => prev + 1); }}
                    title="Dispute"
                    table="disputes"
    ...
    const mobileHeader = <div className="appHeader">
        ...
        <div className="right">
            {
                curAction === "view" && viewOrEditData && context.isAdminResourcePermit(userId, 'alldisputes.edit-dispute') && (<a className="headerButton" onClick={() => setCurAction("edit")}>
    ...
    const addEditComponents = () => {
        if (curAction === "view")
            ...
            return <AddEditAdjustment formItemLayout={formItemLayout} onListClick={onAddEditListClick} onSaveFinish={onAddEditSaveFinish} userId={userId} isMydisputes={false}/>
        else if (curAction === "edit")
            return <AddEditAdjustment formItemLayout={formItemLayout} editIdOrObject={viewOrEditData} onListClick={onAddEditListClick} onSaveFinish={onAddEditSaveFinish} userId={userId} isMydisputes={false}/>
        else return <></>
    }
    const rightButtons =
        <Space><MyButton type='primary' onClick={onAddEditListClick}><i className="fa-solid fa-arrow-left pe-2" ></i>Back</MyButton>
            {(curAction === 'view' || curAction === 'edit') && (<>
                {curAction === 'view' && context.isAdminResourcePermit(userId, 'alldisputes.edit-dispute') && (<MyButton type='primary' onClick={() => { setCurAction("edit") }}><i className="fa-solid fa-pencil pe-2" ></i>Edit</MyButton>)}
                {curAction === 'edit' && (<MyButton type='primary' onClick={() => { setCurAction("view") }}><i className="fa-solid fa-eye pe-2" ></i>View</MyButton>)}
                {
                    context.isAdminResourcePermit(userId, 'alldisputes.delete-dispute') && (<DeleteButton type="outlined" size="small" shape="round" color={red[7]} onFinish={() => { setCurAction("list"); setRefreshTable(prev => prev + 1); }}
                        title="Dispute"
                        table="disputes"
                        //id must,+ give first three colums to display
    ...
    const getSwipeLeftActions = (item) => {
        ...
        if (context.isAdminResourcePermit(userId, 'alldisputes.edit-dispute')) {
            tmpActions.push({
                key: 'edit',
                text: <MyButton type="outlined" size="small" shape="circle"
            ...
            >
                {
                    !context.isMobile && (<>
                        ...
                        <Card
                            ...
                            extra={context.isAdminResourcePermit(userId, 'alldisputes.add-new-dispute') ? <><Space><MyButton onClick={onAddClick} ><i className="fa-solid fa-plus pe-2" ></i>Add Dispute</MyButton>
                            ...
                            <PaginatedTable
                                ...
                                countQuery={"select count(*) as count from disputes a where status=1 " +
                                    context.psGlobal.getWhereClause(filterColumns.current, false)}
                ...
                }
                {
                    context.isMobile && (<>
                        ...
                        <div style={{ display: (curAction === "list") ? 'block' : 'none' }}>
                            {
                                context.isAdminResourcePermit(userId, 'alldisputes.add-new-dispute') && (<>
                                    <FloatButton
                                        type="primary"
                                        shape="circle"
                                        onClick={onAddClick}
                            ...
                            <AvatarMobileInfiniteList
                                ...
                                userId={userId}
                                refresh={refreshTable}
                                countQuery={"select count(*) as count from disputes a where a.status=1 " +
                                    context.psGlobal.getWhereClause(filterColumns.current, false)}
                                ...
                                renderItem={(item, index) => {
                                    return <SwipeAction
                                        rightActions={context.isAdminResourcePermit(userId, 'alldisputes.delete-disputes') ? [
                                            {
                                                key: 'delete',
