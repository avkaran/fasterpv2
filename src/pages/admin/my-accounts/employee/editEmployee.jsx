...
const EditEmployee = (props) => {
  ...
  const LoadBranches = () => {
    var reqData = {
      query_type: "query", //query_type=insert | update | delete | query
      query:
        "select id,branch_name,branch_district from branches where status=1 and branch_status='Active'",
    };
    context.psGlobal
      .apiRequest(reqData, context.adminUser(userId).mode)
      .then((res) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (curAction === "add") {
        ...
        reqData = {
          query_type: "query",
          query:
            "select * from vi_users where username='" +
            username +
            "' and ref_id<>" +
            editId,
        };
              ...
              >
                ...
                >
                  {branches.map((item) => {
                    return (
                      <Select.Option value={item.id}>
                        {item.branch_name}-{item.branch_district}
                      </Select.Option>
                    );
                  })}
                </Select>
            ...
            <Col className="gutter-row" xs={24} xl={12}>
              ...
              >
                <ImageUpload
                  cropRatio="3/2"
                  defaultImage={
                    editData && editData.aadhar_image
                      ? editData.aadhar_image
                      : null
                  }
                  storeFileName={
                    editData && editData.aadhar_image
                  ...
                  onFinish={(fileName) => {
                    addeditFormEmployee.setFieldsValue({
                      employees: { aadhar_image: fileName },
                    });
                  }}
                />
              </FormItem>
